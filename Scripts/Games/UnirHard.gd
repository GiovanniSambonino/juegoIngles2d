...
var difficulty = 'hard'
var title = "match it"
var rondas = 6  # Número total de rondas.
...
var velocidad = 20
var valorNivel = 100
var tiempoCronometro = 60  # Tiempo en segundos del cronómetro.
...
func load_hard_mode_animals():
	...
	if file:
		...
		if typeof(json_result) == TYPE_DICTIONARY and "hard" in json_result:
			animals = json_result["hard"].duplicate(true)
		else:
...
# Pick 18 unique animals for the round
	for i in range(18):
...
func iniciar_juego():
	emit_signal("update_level", str(numeroRondas)+"/6")
